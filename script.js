...
btnContainer.addEventListener("mouseover",function(event){
  let topPos = Math.floor(Math.random()*(window.innerHeight - btnContainer.offsetHeight));
  let leftPos = Math.floor(Math.random()*(window.innerWidth - btnContainer.offsetWidth));
...
function incSeconds(){
  ...
  title.innerHTML = seconds < 2 ? seconds + " second wasted" : seconds + " seconds wasted";
}
...
const gifArr = ["3oFyDpRagf96Uz9rzO","M5myuxwZaK2J2","t6lGyl8QLylqFQJkiA","JIX9t2j0ZTN9S","1d7F9xyq6j7C1ojbC5"];
...
function gifMove(){
  ...
  if(!gif.moving){
    ...
    if( (((gif.xPos + gif.offsetWidth) > window.innerWidth) || gif.xPos < 0 ) || ((gif.yPos > (window.innerHeight - gif.offsetHeight)) || (gif.yPos < 0)) ){
      changeGif();
